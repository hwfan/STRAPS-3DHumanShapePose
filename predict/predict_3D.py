...
def predict_3D(video_path,
               regressor,
               device,
               silhouettes_from='densepose',
               joints2D_predictor=None,
               silhouette_predictor=None,
               smpl=None,
    ...
    for frame_id in tqdm(range(video_length)[:100], desc='frame'):
        ret, image = cap.read()
        ...
        if render_vis:
            ...
            rend_reposed_img = wp_renderer.render(verts=pred_reposed_vertices,
                                                ...
                                                angle=180,
                                                axis=[1, 0, 0])
            if not os.path.isdir(os.path.join(output_dir, 'rend_vis')):
                os.makedirs(os.path.join(output_dir, 'rend_vis'))
            cv2.imwrite(os.path.join(inpuoutput_dirt, 'rend_vis', f'rend_{frame_id}.jpg'), rend_img)
            cv2.imwrite(os.path.join(output_dir, 'rend_vis', f'reposed_{frame_id}.jpg'), rend_reposed_img)
        if save_proxy_vis:
            if not os.path.isdir(os.path.join(output_dir, 'proxy_vis')):
                os.makedirs(os.path.join(output_dir, 'proxy_vis'))
            cv2.imwrite(os.path.join(output_dir, 'proxy_vis', f'silhouette_{frame_id}.jpg'), silhouette_vis)
            cv2.imwrite(os.path.join(output_dir, 'proxy_vis', f'joints2D_{frame_id}.jpg'), joints2D_vis)
